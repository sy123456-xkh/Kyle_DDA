...
export default function WorkspacePage() {
  ...
  const updateManifest = async (
    field: "primary_time_col" | "metric_col" | "metric_agg" | "time_grain",
    value: string
  ) => {
    if (!datasetId) return
  ...
  const runPlaybook = async (playbook: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: `Playbook 执行失败: ${data.detail}` },
        ])
  ...
  const rateTag = (r: number) =>
    r === 0 ? "tag-rate-ok" : r < 0.1 ? "tag-rate-warn" : "tag-rate-bad"

  const numericCols =
    profile?.columns.filter((c) =>
      ["BIGINT", "INTEGER", "DOUBLE", "FLOAT", "DECIMAL", "SMALLINT"].includes(c.type.toUpperCase())
    ) || []
  const allCols = profile?.columns || []

  const showChart =
    activeResult?.chart &&
    ["line", "bar", "pie"].includes(activeResult.chart.type) &&
    activeResult.chart.data.length > 0
        ...
        <ErrorBoundary>
          <aside className="w-80 flex-shrink-0 border-r border-[var(--border)] flex flex-col bg-[var(--bg-secondary)]">
            ...
            <div className="p-4 border-b border-[var(--border)]">
                  ...
                  <div className="flex flex-col items-center gap-2 w-full">
                    ...
                    <div className="w-full h-1.5 bg-[var(--bg-primary)] rounded-full overflow-hidden mt-1">
                      <div
                        ...
                        style={{ width: "75%", transition: "width 0.3s ease-out" }}
                      />
                    </div>
                  </div>
                ) : (
              ...
              {uploadError && (
                <p className="mt-2 text-xs text-[var(--danger)] animate-fade-in">{uploadError}</p>
              )}
            </div>
            ...
            {profile && manifest && (
              <div className="p-4 border-b border-[var(--border)] animate-slide-up">
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <select
                      ...
                      value={manifest.primary_time_col || ""}
                      onChange={(e) => updateManifest("primary_time_col", e.target.value)}
                    >
                  ...
                  <div>
                    ...
                    <select
                      className="select-field"
                      value={manifest.metric_col || ""}
                      onChange={(e) => updateManifest("metric_col", e.target.value)}
                    >
                      <option value="">-- 未选择 --</option>
                      {numericCols.map((c) => (
                        <option key={c.name} value={c.name}>
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1">
                      ...
                      <select
                        className="select-field"
                        value={manifest.metric_agg || "sum"}
                        onChange={(e) => updateManifest("metric_agg", e.target.value)}
                      >
                    ...
                    <div className="flex-1">
                      ...
                      <select
                        className="select-field"
                        value={manifest.time_grain || "day"}
                        onChange={(e) => updateManifest("time_grain", e.target.value)}
                      >
                ...
                {/* Playbook */}
                <h3 className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mt-4 mb-2">
                  Playbook
                </h3>
                <div className="space-y-2">
                  <button
                    className="playbook-btn"
                    onClick={() => runPlaybook("trend")}
                    disabled={querying}
                  >
                    <span className="pb-icon trend">
                      <Ico d="M22 12l-4-4v3H3v2h15v3l4-4z" size={14} />
                    </span>
                    <span>趋势分析</span>
                  </button>
                  <button
                    className="playbook-btn"
                    onClick={() => runPlaybook("topn")}
                    disabled={querying}
                  >
                    <span className="pb-icon topn">
                      <Ico d="M18 20V10M12 20V4M6 20v-6" size={14} />
                    </span>
                    <span>Top N 排行</span>
                  </button>
                  <button
                    className="playbook-btn"
                    onClick={() => runPlaybook("cross")}
                    disabled={querying}
                  >
                    <span className="pb-icon cross">
                      <Ico d="M3 3v18h18M7 16l4-4 4 4 5-6" size={14} />
                    </span>
                    <span>交叉分析</span>
                  </button>
                </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <div className="flex-1 h-1 bg-[var(--bg-primary)] rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.max(miss * 100, 0.5)}%`,
                                background:
                                  miss === 0 ? "#34d399" : miss < 0.1 ? "#fbbf24" : "#f87171",
                              }}
                            />
                          </div>
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {samp.slice(0, 3).map((v, j) => (
                            <span
                              key={j}
                              className="text-xs px-1.5 py-0.5 rounded bg-[var(--bg-primary)] text-[var(--text-muted)] truncate max-w-[90px]"
                              title={String(v)}
                            >
                              {String(v)}
                            </span>
                ...
                <div className="space-y-1">
                  {history.map((h, i) => (
                    <div key={i} className="history-item" onClick={() => replayHistory(h)}>
                      <span className="text-[var(--text-muted)] flex-shrink-0 w-[120px] truncate">
                        {h.timestamp}
                      </span>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-[var(--text-muted)] gap-3">
                  ...
                  <p className="text-sm">
                    {datasetId ? "输入问题或使用 Playbook 开始分析" : "请先上传 CSV 文件"}
                  </p>
                </div>
              )}
              {messages.map((msg, i) => (
                ...
                >
                  <div
                    className={`max-w-[75%] px-4 py-2.5 text-sm ${msg.role === "user" ? "chat-bubble-user" : "chat-bubble-assistant"}`}
                  >
                    <p>{msg.content}</p>
                    {msg.result && (
                      <button
            ...
            <div className="p-4 border-t border-[var(--border)] bg-[var(--bg-secondary)]">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={!datasetId || !question.trim() || querying}
                  onClick={handleQuery}
                >
              ...
              {datasetId && (
                <div className="flex gap-2 mt-2">
                  {["这个数据有多少行？", "展示所有数据", "count"].map((q) => (
                    <button key={q} className="btn-ghost text-xs" onClick={() => setQuestion(q)}>
                      {q}
                    </button>
                  ))}
                </div>
                  ...
                  <div className="p-4 border-b border-[var(--border)]">
                    ...
                    <div className="mt-3">
                      <ChartConfig
                        ...
                        columns={
                          activeResult.rows.length > 0 ? Object.keys(activeResult.rows[0]) : []
                        }
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-1">
                      <button
                        ...
                        onClick={() => copySQL(activeResult.sql)}
                      >
                        {copied ? (
                          <>
                            <Ico d="M20 6L9 17l-5-5" size={12} className="stroke-[#34d399]" />{" "}
                            已复制
                          </>
                        ) : (
                          <>
                            <Ico
                              d="M9 9h13v13H9zM5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"
                              size={12}
                            />{" "}
                            复制
                          </>
                ...
                <div className="flex-1 overflow-auto p-4">
                  {activeResult.rows.length > 0 ? (
                    <table className="data-table">
                      <thead>
                        <tr>
                          {Object.keys(activeResult.rows[0]).map((k) => (
                            <th key={k}>{k}</th>
                          ))}
                        </tr>
                      </thead>
