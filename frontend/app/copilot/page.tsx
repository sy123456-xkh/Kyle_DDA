...
export default function CopilotPage() {
  ...
  const mockFields = ["GMV", "ARPU", "Region", "Payment_Type", "User_ID", "Category", "Timestamp", "Store_ID"];
  ...
  const handleSend = async () => {
    ...
    if (!datasetId) {
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: "请先在 Data Hub 上传 CSV 数据集，然后返回 Copilot 进行分析。", time: now() },
        ]);
        setQuerying(false);
      }, 500);
      return;
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content: res.ok
            ...
            : `查询失败: ${data.detail}`,
          time: now(),
          actions: res.ok ? [{ label: "Export CSV", icon: "download" }, { label: "Share Report", icon: "share" }] : undefined,
        },
      ]);
    } catch {
      setMessages((prev) => [...prev, { role: "assistant", content: "网络错误，请检查后端是否启动。", time: now() }]);
    } finally {
      setQuerying(false);
    }
  };
      ...
      <nav className="border-b border-gray-200 bg-white">
        <div className="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Link href="/" className="text-xl font-bold text-amber-700">Kyle Studios BI Copilot</Link>
            <div className="flex gap-6 text-sm">
              <span className="text-amber-600 border-b-2 border-amber-600 pb-1 font-medium">Workspace</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-8 h-8 rounded-full bg-amber-700 flex items-center justify-center text-white text-sm font-medium">K</div>
          </div>
        </div>
      </nav>
        ...
        <aside className="w-64 border-r border-gray-200 flex flex-col bg-white">
          <div className="p-5">
            ...
            <div className="flex items-center gap-1.5 mb-4">
              ...
              <span className="text-xs text-gray-500 uppercase tracking-wide">Project Alpha Active</span>
            </div>
            ...
            <p className="text-[10px] text-gray-400 uppercase tracking-wide mb-3">Available Fields</p>
            <div className="flex flex-wrap gap-2">
              {mockFields.map((f, i) => (
                <span key={f} className={`text-xs px-3 py-1 rounded-full border ${i === 0 ? 'border-amber-500 text-amber-600 bg-amber-50' : 'border-gray-200 text-gray-600'}`}>
                  {f}
                </span>
              ))}
            </div>
          ...
          <div className="mt-auto p-5 space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400 px-1 pt-2">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" strokeWidth={1.5} /><path strokeLinecap="round" strokeWidth={1.5} d="M12 16v-4M12 8h.01" /></svg>
              Help
            </div>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-gray-50">
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`max-w-[70%] ${msg.role === "user" ? "bg-amber-50 border border-amber-200" : "bg-white border border-gray-200"} rounded-xl px-4 py-3`}>
                  <p className="text-sm text-gray-800">{msg.content}</p>
                  {msg.actions && (
                    <div className="flex gap-4 mt-3">
                      {msg.actions.map((a) => (
                        <button key={a.label} className="text-xs text-amber-600 font-medium flex items-center gap-1 hover:text-amber-700">
          ...
          <div className="p-4 border-t border-gray-200 bg-white">
            <div className="flex gap-2 items-center">
              ...
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19V5M5 12l7-7 7 7" /></svg>
              </button>
            </div>
          </div>
        </main>
        ...
        <aside className="w-[400px] border-l border-gray-200 flex flex-col bg-white overflow-y-auto">
          ...
          <div className="p-5 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-amber-600" fill="currentColor" viewBox="0 0 20 20"><path d="M2 11a1 1 0 011-1h2a1 1 0 011 1v5a1 1 0 01-1 1H3a1 1 0 01-1-1v-5zM8 7a1 1 0 011-1h2a1 1 0 011 1v9a1 1 0 01-1 1H9a1 1 0 01-1-1V7zM14 4a1 1 0 011-1h2a1 1 0 011 1v12a1 1 0 01-1 1h-2a1 1 0 01-1-1V4z" /></svg>
            ...
            <div className="h-48 flex items-end gap-3 px-2 mb-4">
              {[
                ...
                <div key={bar.label} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div className={`w-full ${bar.color} rounded-t`} style={{ height: `${(bar.value / 72.4) * 140}px` }} />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-gray-50 rounded-xl p-3 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center">
                  <svg className="w-5 h-5 text-amber-600" fill="currentColor" viewBox="0 0 20 20"><path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4zM18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z" /></svg>
                </div>
                <div>
          ...
          <div className="p-5 border-b border-gray-200">
            ...
            <div className="space-y-3">
              {[
                { n: 1, text: 'SP 地区 GMV 遥遥领先，建议加大该地区的运力投入，优化"最后一公里"配送效率。' },
                { n: 2, text: "RJ 与 MG 地区增长势头强劲，可考虑通过区域联动大促，进一步释放市场潜力。" },
                { n: 3, text: "BA 地区的客单价偏低，建议优化商品结构，引入更多高客单价品类（如电子产品或高端家居）。" },
              ].map((item) => (
                <div key={item.n} className="flex gap-3">
                  <span className="w-6 h-6 rounded-full bg-amber-100 text-amber-600 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{item.n}</span>
          ...
          <div className="p-5">
            <div className="bg-gray-800 rounded-xl p-4 text-white">
              ...
              <span className="inline-block mt-2 text-[10px] px-2 py-0.5 border border-gray-600 rounded text-gray-400 uppercase tracking-wide">Token Optimized</span>
            </div>
          </div>
        </aside>
      </div>
