...
const TYPE_ICONS: Record<string, { label: string; d: string }> = {
  ...
  pie: {
    label: "饼图",
    d: "M12 2a10 10 0 0 1 10 10h-10V2zM12 12L2.05 14.5A10 10 0 0 0 12 22a10 10 0 0 0 9.95-8.5L12 12z",
  },
}
        ...
        <div className="px-3 pb-3 space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-1.5">
              {(Object.entries(TYPE_ICONS) as [string, { label: string; d: string }][]).map(
                ([type, { label, d }]) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              className="flex-1 bg-[var(--bg-secondary)] border border-[var(--border)] rounded px-2 py-1 text-xs text-[var(--text-primary)] outline-none focus:border-[var(--accent)] transition-colors"
              value={
                (Array.isArray(overrides.y) ? overrides.y[0] : overrides.y) ??
                (Array.isArray(spec.y) ? spec.y[0] : spec.y) ??
                ""
              }
