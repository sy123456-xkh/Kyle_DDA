...
export default function ChartView({ spec, overrides, className }: ChartViewProps) {
  ...
  const shouldRender = merged.type !== "table" && merged.data && merged.data.length > 0
  ...
  useEffect(() => {
    ...
    import("echarts").then((echarts) => {
      ...
      const legendCfg = {
        orient: (lp === "left" || lp === "right" ? "vertical" : "horizontal") as
          | "vertical"
          | "horizontal",
  ...
  return <div ref={domRef} className={className} style={{ width: "100%", height: "260px" }} />
}
